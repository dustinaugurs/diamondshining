...
@section ('title', trans('labels.backend.orders.enquiry'))

@section('page-header')
    <h1>{{ trans('labels.backend.orders.enquiry') }}</h1>
@endsection

@section('content')
    <div class="box box-info">
        <div class="box-header with-border">
            <h3 class="box-title">{{ trans('labels.backend.orders.enquiry') }}</h3>
...
<th>Date 
 ...
 @include('backend.ordermanagements.enquiries_component')  
      ...
      var jsondata='', symbol='', currency='', i, outputdata='', orderStatus='', cost_ex_VAT='', salePrice_ex_VAT='', salePrice_inc_VAT='', paymentStatus='' , payment_status='', date='', get_order_status='', checkStatus='', check_status='' ;
          //console.log('date value_'+$(this).val());
           ...
           var allordstatus = [1];
            ...
            $.ajax({
                type:"GET",
                url:"{{ url('admin/enqaddateAndPaymentFilter') }}",
                data:data,
                cache:false,
				       // dataType:'json',
                beforeSend: function(){
    ...
    $('body').on('change', '.orderstatuptOrder, .paystatupdateOrder', function(){
      var  payment_status='',  order_status='', date='', check_status='', prid=''  ;
         ...
         var allordstatus = [1];
            ...
            $.ajax({
                type:"POST",
                url:"{{ url('admin/enqadOrderStatusAndPaymentUpdate') }}",
                data:data,
                cache:false,
				        //dataType:'json',
                beforeSend: function(){
...
$('body').on('click', '.checkstatupdatetwothree', function(){
      var payment_status='', order_status='', date='', check_status='', deliveryCost='', prid='', trackingid='';
      ...
      $('#myModal').modal('hide');
         ...
         var allordstatus = [1];
			...
			var data = 'date='+date+'&pid='+prid+'&payment_status='+payment_status+'&order_status='+order_status+'&check_status='+check_status+'&deliveryCost='+deliveryCost+'&allordstatus='+allordstatus+'&trackingid='+trackingid+'&_token={{ csrf_token() }}';
