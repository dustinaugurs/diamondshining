...
<div class="row">
  ...
  <div class="printbox">
  <h4>User Log Details of Customer  <span id="printdetail" onclick="window.print();"><i class="fa fa-print" aria-hidden="true"></i></span> <span onclick="window.history.back();"><i class="fa fa-arrow-circle-left"></i></span></h4>
  ...
  <tr>
    ...
    <td>{{$customers->login_time}}</td>
  </tr>

  <tr>
    <td><strong>Logout</strong></td> 
    <td>{{$customers->logout_time}}</td>
  </tr>
  ...
  <tr>
    ...
    <td>
    <strong>
        @if(!empty($customers->logout_time))
        {{ date('G:i:s', strtotime($customers->logout_time) - strtotime($customers->login_time)) }} Hrs.
        @else
        00:00:00 Hrs.
        @endif
    </strong>
