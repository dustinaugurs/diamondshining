...
class ProductController extends Controller
{
	...
	public function ourProduct(Request $request,  $base = null){
		...
		//$products = DB::table('diamond_feeds')->paginate(15);
	//	$products = DB::table('diamond_feeds')->select('*');
       ...
       return view('frontend.pages.our_products', compact('products' ,$products , 'current_currency' , $current_currency ,'symbol' ,$symbol, 'total_diamond_found' , $total_diamond_found));
	  }
   
	 public function SearchourProduct(Request $request,  $base = null){
		//$stockId = "LD200110653, LD200110224, G431-138852";
		$stockId = $request->search_stock_number; 
		//$sid = $stockId;  
		$sdata_id = '';
		$sid = explode(",", trim($stockId, ","));
		foreach($sid as $si){
			$sdata_id .= '"'.$si.'"'.",";
		}
		$xa = explode(",", rtrim($sdata_id,','));
		//  print_r($xa);
		// die;

		//$stockId = "LD200110653, LD200110224, G431-138852";
		//$strToArray = explode(",",$stockId);
		//echo '<pre>';
		//print_r($stockId);
		//print_r($strToArray); die;
	...
	//	$products = DB::table('diamond_feeds')->select('*');
        //$aa=  datatables()->of($products)
            //->make(true);
	   // print_r($aa); die;
	   $total_diamond_found = DB::table('diamond_feeds')->where('stock_id', $stockId)->count();
		//print_r($total_diamond_found); die;
